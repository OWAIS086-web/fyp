...
def wiki(notepad):
    ...
    try:
        ...
        w.destroy()
    w.configure(background="#1b1b1b"), w.title("SM-Pedia"), w.resizable(False, False)
...
def SMscrape(notepad):
    ...
    w.configure(background="#1b1b1b")
    w.title("SM-AI (Web Scraping)")
    ...
    ttk.Button(w, text="Copy Data", command=copy).place(x=295, y=460)

    try:
        # Replace 'https://en.wikipedia.org/wiki/' with the Wikipedia URL and the 'query' with the specific topic you want to search for.
        url = f"https://en.wikipedia.org/wiki/{query}"
        response = requests.get(url)

        if response.status_code == 200:
            soup = BeautifulSoup(response.content, "html.parser")
            # Perform web scraping and extract the data you need from the 'soup' object
            # For example, you can find specific HTML elements and extract their content.
            # Below is just a placeholder example, you need to customize it as per your requirement.
            data = soup.find_all("p")
            count = 0
            for item in data:
                count += 1
                info = f"\nResult {count}:\n{item.get_text()}\n"
                text_w.insert(END, info)

                if count == 100:  # Display up to 5 results (you can change this value)
                    break
        else:
...
def save_as(self):
    ...
    title = self.tabs[curr_tab].file_name + " ~ SM PAD++"
